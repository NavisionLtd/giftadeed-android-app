...
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.PhotoViewerActivity;
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.PreferenceUtils;
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.TextUtils;
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.UrlPreviewInfo;
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.VideoPlayer;
import giftadeed.kshantechsoft.com.giftadeed.SendBirdChat.utils.WebUtils;
        ...
        rootView.getRootView().setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (event.getAction() == KeyEvent.ACTION_UP && keyCode == KeyEvent.KEYCODE_BACK) {
                    /*Intent i = new Intent(getActivity(), TaggedneedsActivity.class);
                    i.putExtra("FRAGMENT", strStaicRedirectionPage);
                    startActivity(i);
                    ((Activity) getActivity()).overridePendingTransition(0, 0);*/

                    // move to group channel list fragment
                    GroupChannelListFragment groupChannelListFragment = new GroupChannelListFragment();
                    fragmgr.beginTransaction().replace(R.id.content_frame, groupChannelListFragment).commit();
                    return true;
                }
                return false;
            }
...
//                    updateActionBarTitle();
                    TaggedneedsActivity.updateTitle("" + mChannel.getName());   //set name of group channel name
                }
            });
        } else {
...
//                    updateActionBarTitle();
    ...
    private void onFileMessageClicked(FileMessage message) {
        ...
        String mMediaFilter = message.getUrl().toLowerCase();
        if (type.startsWith("image")) {
            Intent i = new Intent(getActivity(), PhotoViewerActivity.class);
            i.putExtra("url", message.getUrl());
            i.putExtra("type", message.getType());
            startActivity(i);
        } else if (type.startsWith("application/octet-stream") && (mMediaFilter.contains(".mp4") || mMediaFilter.contains(".3GP"))) {
            Intent intent = new Intent(getActivity(), VideoPlayer.class);
            intent.putExtra("url", message.getUrl());
            intent.putExtra("name", message.getName());
            startActivity(intent);
        } else if (type.startsWith("application/octet-stream") && mMediaFilter.contains(".mp3") || mMediaFilter.contains(".m4a") || mMediaFilter.contains(".aac") || mMediaFilter.contains(".amr")) {
            Intent intent = new Intent(getActivity(), Activity_Audio.class);
            intent.putExtra("url", message.getUrl());
            intent.putExtra("name", message.getName());
            startActivity(intent);
        } else {
...
//            GroupChannelActivity.txtgrpheading.setText(""+mChannel.getName());   //set name of group channel name
        }
    }
}
